...
namespace Cedar {
    namespace Matchers {
        ...
        template < typename
        T >
        const ActualValue<T> operator, (
        const T & actualValue,
        const ActualValueMarker & marker) {
            return ActualValue < T > (marker.fileName, marker.lineNumber, actualValue);
        }

        template < typename
        T >
        const ActualValueMatchProxy<T> operator, (
        const ActualValue<T> & actualValue, bool negate) {
        ...
        MatcherType >
        void operator, (
        const ActualValueMatchProxy<T> & matchProxy,
