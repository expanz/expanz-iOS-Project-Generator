...
- (void) writeObjectValueToInvocationReturnValue:(NSInvocation*)anInvocation {
    ...
    if (KWStringHasWordPrefix(selectorString, @"alloc") || KWStringHasWordPrefix(selectorString, @"new") ||
        KWStringHasWord(selectorString, @"copy") || KWStringHasWord(selectorString, @"Copy")) {
        [self.value retain];
    }
#endif
}
