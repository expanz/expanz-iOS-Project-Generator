...
namespace Cedar {
    namespace Matchers {
        ...
        template < typename
        T > BeLessThan < T > be_less_than(
        const T & expectedValue) {
            return BeLessThan < T > (expectedValue);
        }

        template < typename
        T > BeLessThan < T > ::BeLessThan(
        const T & expectedValue)
        ...
        template < typename
        T > template < typename
        U > bool BeLessThan<T>::matches(const U & actualValue) const {
...
#pragma mark operators
        template < typename
        T, typename
        U > bool operator<(const ActualValue<T> & actualValue, const U & expectedValue) {
            return actualValue.to < expectedValue;
        }

        template < typename
        T, typename
        U > bool operator<(const ActualValueMatchProxy<T> & actualValueMatchProxy, const U & expectedValue) {
