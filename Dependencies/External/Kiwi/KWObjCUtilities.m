...
BOOL KWObjCTypeIsSignedIntegral(const char* objCType) {
    return strcmp(objCType, @encode(char)) == 0 || strcmp(objCType, @encode(int)) == 0 ||
        strcmp(objCType, @encode(short)) == 0 || strcmp(objCType, @encode(long)) == 0 ||
        strcmp(objCType, @encode(long long)) == 0;
}

BOOL KWObjCTypeIsUnsignedIntegral(const char* objCType) {
    return strcmp(objCType, @encode(unsigned char)) == 0 || strcmp(objCType, @encode(unsigned int)) == 0 ||
        strcmp(objCType, @encode(unsigned short)) == 0 || strcmp(objCType, @encode(unsigned long)) == 0 ||
...
BOOL KWObjCTypeIsPointerLike(const char* objCType) {
    return KWObjCTypeIsObject(objCType) || KWObjCTypeIsCharString(objCType) || KWObjCTypeIsClass(objCType) ||
        KWObjCTypeIsSelector(objCType) || KWObjCTypeIsPointerToType(objCType);
}
