...
#pragma mark Generic
            template < typename
            T, typename
            U > bool compare_equal(const T & actualValue, const U & expectedValue) {
                if (strcmp(@encode(T), "@") == 0 && strcmp(@encode(U), "@") == 0) {
                    return [reinterpret_cast<const id &>(actualValue)
                        isEqual:reinterpret_cast<const id &>(expectedValue)];
                }
                else {
...
#pragma mark NSNumber
            ...
            template < typename
            U > bool compare_equal(NSNumber* const actualValue, const U & expectedValue) {
                return [actualValue floatValue] == expectedValue;
            }

            template < typename
            T > bool compare_equal(const T & actualValue, NSNumber* const expectedValue) {
                return compare_equal(expectedValue, actualValue);
            }
