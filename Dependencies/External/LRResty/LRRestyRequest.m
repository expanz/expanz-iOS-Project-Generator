...
- (void) setQueryParameters:(NSDictionary*)parameters; {
    ...
    NSURL* URLWithParameters = [NSURL
        URLWithString:[NSString stringWithFormat:@"%@?%@", [_URLRequest.URL absoluteString],
                                                 [parameters LR_stringWithFormEncodedComponents]]];
...
- (void) setBasicAuthUsername:(NSString*)username password:(NSString*)password
          useCredentialSystem:(BOOL)useCredential; {
    if (useCredential) {
        credential =
            [[NSURLCredential credentialWithUser:username password:password persistence:NSURLCredentialPersistenceNone]
                retain];
    }
    else {
        ...
        [self addHeader:@"Authorization"
                  value:[NSString stringWithFormat:@"Basic %@", [credentialData LR_base64EncodedString]]];
    }
}
...
- (void) finish; {
    LRRestyResponse* restResponse = [[LRRestyResponse alloc]
        initWithStatus:[(NSHTTPURLResponse*) self.URLResponse statusCode] responseData:self.responseData
               headers:[(NSHTTPURLResponse*) self.URLResponse allHeaderFields] originalRequest:self];
...
- (void) connection:(NSURLConnection*)connection didFailWithError:(NSError*)error {
    ...
    LRRestyResponse
        * restResponse = [[LRRestyResponse alloc] initWithStatus:0 responseData:nil headers:nil originalRequest:self];
    [delegate restyRequest:self didFinishWithResponse:restResponse];
    [restResponse release];
