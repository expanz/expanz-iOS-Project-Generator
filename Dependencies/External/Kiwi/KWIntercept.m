...
IMP KWForwardingImplementationForMethodEncoding(const char* encoding) {
    ...
    if (*[signature methodReturnType] == '{' && [signature methodReturnLength] > stretLengthThreshold) {
        NSLog(
            @"Warning: The Objective-C runtime appears to have bugs when forwarding messages with certain struct layouts as return types, so if a crash occurs this could be the culprit");
...
Class KWSetupObjectInterceptSupport(id anObject) {
    ...
    if (IsTollFreeBridged(objectClass, anObject)) {
        [NSException raise:@"KWTollFreeBridgingInterceptException"
                    format:@"Attempted to stub object of class %@. Kiwi does not support setting expectation or stubbing methods on toll-free bridged objects.",
...
void KWSetupMethodInterceptSupport(Class interceptClass, SEL aSelector) {
    ...
    if (method == nil) {
        [NSException raise:NSInvalidArgumentException
                    format:@"cannot setup intercept support for -%@ because there is no such method exists",
