...
#pragma mark class ActualValueMatchProxy
        ...
        ActualValueMatchProxy
        {
            private:
                template < typename
            U > ActualValueMatchProxy(
            const ActualValueMatchProxy<U> &);
            template < typename
            U > ActualValueMatchProxy & operator =(const ActualValueMatchProxy<U> &);
            ...
            template < typename
            MatcherType >
            void operator()(const MatcherType &) const;
        ...
        template < typename
        T > ActualValueMatchProxy < T > ::ActualValueMatchProxy(
        const ActualValue<T> & actualValue, bool negate /*= false */)
        ...
        template < typename
        T > ActualValueMatchProxy < T > ActualValueMatchProxy < T > ::negate() const {
...
#pragma mark class ActualValue
        ...
        ActualValue
        {
            ...
            const ActualValue<U> &);
            template < typename
            U > ActualValue & operator =(const ActualValue<U> &);
        ...
        template < typename
        T > ActualValue < T > ::ActualValue(
        const char* fileName,
        int lineNumber,
        const T & value) : fileName_(fileName), lineNumber_(lineNumber), value_(value), to(*this), to_not(*this, true)
        {
        }
        ...
        inline void CDR_fail(const char* fileName, int lineNumber, NSString* reason) {
            [[CDRSpecFailure
                specFailureWithReason:reason fileName:[NSString stringWithUTF8String:fileName] lineNumber:lineNumber]
                raise];
        }
