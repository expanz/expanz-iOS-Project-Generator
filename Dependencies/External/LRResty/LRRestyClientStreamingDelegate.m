...
+ (id) delegateWithBlock:(LRRestyResponseBlock)theBlock dataHandler:(LRRestyStreamingDataBlock)dataBlock
            errorHandler:(LRRestyStreamingErrorBlock)errorBlock {
    return [[[self alloc] initWithBlock:theBlock dataHandler:dataBlock errorHandler:errorBlock] autorelease];
}

- (id) initWithBlock:(LRRestyResponseBlock)theBlock dataHandler:(LRRestyStreamingDataBlock)dataBlock
        errorHandler:(LRRestyStreamingErrorBlock)errorBlock {
...
- (void) restyRequest:(LRRestyRequest*)request didFinishWithResponse:(LRRestyResponse*)response {
    if ([response status] > 200 && errorHandler != nil) {
        NSError* error = [NSError
            errorWithDomain:LRRestyClientStreamingErrorDomain code:LRRestyStreamingErrorUnsuccessfulResponse
