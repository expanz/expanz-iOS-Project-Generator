...
- (LRRestyRequest*) post:(NSString*)urlString payload:(id)payload
                delegate:(id<LRRestyClientResponseDelegate>)delegate; {
    return [self post:urlString payload:payload headers:nil delegate:delegate];
}

- (LRRestyRequest*) post:(NSString*)urlString payload:(id)payload headers:(NSDictionary*)headers
                delegate:(id<LRRestyClientResponseDelegate>)delegate; {
    return [HTTPClient POST:[NSURL URLWithString:urlString] payload:payload headers:headers
                   delegate:[LRRestyClientProxyDelegate proxyForClient:self responseDelegate:delegate]];
}
...
- (LRRestyRequest*) post:(NSString*)urlString payload:(id)payload headers:(NSDictionary*)headers
               withBlock:(LRRestyResponseBlock)block; {
    return [HTTPClient POST:[NSURL URLWithString:urlString] payload:payload headers:headers
                   delegate:[LRRestyClientBlockDelegate delegateWithBlock:block]];
}
...
- (LRRestyResponse*) post:(NSString*)urlString payload:(id)payload; {
    return [self performAsynchronousBlockWithTimeout:globalTimeoutInterval + 1
                            andReturnResultWhenReady:^(id* result, dispatch_semaphore_t semaphore) {
