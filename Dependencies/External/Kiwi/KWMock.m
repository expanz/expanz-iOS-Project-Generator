...
- (id) initAsNullMock:(BOOL)nullMockFlag withName:(NSString*)aName forClass:(Class)aClass
...
- (id) stubAndReturn:(id)aValue {
    NSDictionary
        * userInfo = [NSDictionary dictionaryWithObjectsAndKeys:StubTag, ExpectOrStubTagKey, aValue, StubValueKey, nil];
    return [KWInvocationCapturer invocationCapturerWithDelegate:self userInfo:userInfo];
}
...
- (void) forwardInvocation:(NSInvocation*)anInvocation {
    ...
    KWMessagePattern* messagePattern = [KWMessagePattern messagePatternFromInvocation:anInvocation];
    [NSException raise:@"KWMockException" format:@"%@ received unexpected message -%@", [self namePhrase],
...
- (BOOL) mockedProtocolConformsToProtocol:(Protocol*)aProtocol {
    ...
    return protocol_isEqual(self.mockedProtocol, aProtocol) ||
        protocol_conformsToProtocol(self.mockedProtocol, aProtocol);
}
...
- (BOOL) respondsToSelector:(SEL)aSelector {
    return [self mockedClassRespondsToSelector:aSelector] || [self mockedProtocolRespondsToSelector:aSelector] ||
        [super respondsToSelector:aSelector];
}

- (BOOL) conformsToProtocol:(Protocol*)aProtocol {
    return [self mockedClassConformsToProtocol:aProtocol] || [self mockedProtocolConformsToProtocol:aProtocol] ||
