...
+ (NSDictionary*) LR_dictionaryWithFormEncodedString:(NSString*)encodedString {
    ...
    for (NSString* kvp in pairs) {
        ...
        if (!key || !val) {
            continue;
        } // I'm sure this will bite my arse one day
...
- (void) formEncodeObject:(id)object usingKey:(NSString*)key subKey:(NSString*)subKey intoArray:(NSMutableArray*)array {
    ...
    if (subKey) {
        objectKey = [NSString stringWithFormat:@"%@[%@]", [key description],
                                               [[subKey description] LR_stringByEscapingForURLQuery]];
    }
    else {
    ...
    if ([object respondsToSelector:@selector(stringByEscapingForURLQuery)]) {
        ...
        [array addObject:[NSString stringWithFormat:@"%@=%@", objectKey,
                                                    [[object description] LR_stringByEscapingForURLQuery]]];
    }
}
