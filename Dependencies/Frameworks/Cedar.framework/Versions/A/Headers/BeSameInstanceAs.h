...
namespace Cedar {
    namespace Matchers {
        ...
        template < typename
        T > BeSameInstanceAs < T > be_same_instance_as(T *
        const expectedValue) {
            return BeSameInstanceAs < T > (expectedValue);
        }

        template < typename
        T > BeSameInstanceAs < T > ::BeSameInstanceAs(T *
        const expectedValue)
...
#pragma mark Generic
        template < typename
        T > template < typename
        U > bool BeSameInstanceAs<T>::matches(const U & actualValue) const {
        ...
        template < typename
        T > template < typename
        U > bool BeSameInstanceAs<T>::matches(U* const & actualValue) const {
