...
namespace Cedar {
    namespace Matchers {
        ...
        template < typename
        T > BeGTE < T > be_greater_than_or_equal_to(
        const T & expectedValue) {
        ...
        T >
            ...
            return [NSString stringWithFormat:@"be greater than or equal to <%@>",
                                              Stringifiers::string_for(expectedValue_)];
        }

        template < typename
        T > template < typename
        U > bool BeGTE<T>::matches(const U & actualValue) const {
            this->build_failure_message_start(actualValue);
            return Comparators::compare_greater_than(actualValue, expectedValue_) ||
                Comparators::compare_equal(actualValue, expectedValue_);
        }

#pragma mark operators
        template < typename
        T, typename
        U > bool operator>=(const ActualValue<T> & actualValue,
        ...
        template < typename
        T, typename
        U > bool operator>=(const ActualValueMatchProxy<T> & actualValueMatchProxy,
        const U & expectedValue) {
