...
namespace Cedar {
    namespace Matchers {
        ...
        BeCloseTo : public Base {
            ...
            protected:
                virtual NSString
            *failure_message_end() const;

            private:
                template < typename
            U, typename
            V > bool subtractable_types_match(const U &, const V &) const;
        ...
        template < typename
        T > BeCloseTo < T > be_close_to(
        const T & expectedValue) {
            return BeCloseTo < T > (expectedValue);
        }

        template < typename
        T > BeCloseTo < T > ::BeCloseTo(
        const T & expectedValue)
        ...
        template < typename
        T > BeCloseTo < T > &BeCloseTo < T > ::within(
        float threshold) {
        ...
        T >
            ...
            return [NSString stringWithFormat:@"be close to <%@> (within %@)", Stringifiers::string_for(expectedValue_),
                                              Stringifiers::string_for(threshold_)];
        }

        template < typename
        T > template < typename
        U, typename
        V > bool BeCloseTo<T>::subtractable_types_match(const U & actualValue, const V & expectedValue) const {
...
#pragma mark Generic
        template < typename
        T > template < typename
        U > bool BeCloseTo<T>::matches(const U & actualValue) const {
            return this->subtractable_types_match(actualValue, expectedValue_);
        }

#pragma mark NSNumber
        template < typename
        T > bool BeCloseTo<T>::matches(NSNumber* const & actualValue) const {
